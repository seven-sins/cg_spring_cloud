...
public class Kafka010Example {
	...
	public static void main(String[] args) throws Exception {
		...
		args = new String[] {"--input-topic", "testTopic", "--bootstrap.servers", "192.168.0.205:9092", "--zookeeper.connect", "192.168.0.205:2181", "--group.id", "myconsumer" };
		...
		DataStream<KafkaEvent> input = env
				.addSource(new FlinkKafkaConsumer010<>(parameterTool.getRequired("input-topic"), new KafkaEventSchema(),
						parameterTool.getProperties()).assignTimestampsAndWatermarks(new CustomWatermarkExtractor()))
				.keyBy("word").map(new RollingAdditionMapper());
	 ...
	 * frequency count of a key. The current total count is keyed state managed by
	 * Flink.
	 */
	private static class RollingAdditionMapper extends RichMapFunction<KafkaEvent, KafkaEvent> {
		...
		public void open(Configuration parameters) throws Exception {
			currentTotalCount = getRuntimeContext()
